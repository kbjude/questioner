...
def test_cannot_add_tag_which_does_not_exist_to_meetup(api_client, db, meetup1, user1):
...
def test_cannot_add_tag_to_meetup_with_invalid_meetup_id(api_client, db, admin_user, a_tag):
    api_client.force_authenticate(user=admin_user)
    response = api_client.post(
...
def test_user_cannot_remove_a_tag_not_added_by_them_to_a_meetup(api_client, db, meetup1, user2, tagged_meetup):
...
def test_user_can_remove_a_tag_added_by_them_to_a_meetup(api_client, db, meetup1, user1, tagged_meetup):
    api_client.force_authenticate(user=user1)
    response = api_client.delete(
        reverse('meetingtag', kwargs={'meeting_id': tagged_meetup.meetup.id, 'tag_id': tagged_meetup.tag.id}))
...
def test_admin_user_can_remove_any_tag_from_a_meetup(api_client, db, meetup1, admin_user, tagged_meetup):
    api_client.force_authenticate(user=admin_user)
    response = api_client.delete(
        reverse('meetingtag', kwargs={'meeting_id': tagged_meetup.meetup.id, 'tag_id': tagged_meetup.tag.id}))
    assert response.status_code == 200
