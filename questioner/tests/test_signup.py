...
class TestLogin(APITestCase):
    ...
    def test_signup_user_with_valid_data(self):
        ...
        """
        data = {"username": "bisonlou", "password": "Pa$$word123", "email": "bisonlou@gmail.com"}
        url = reverse("signup")
        response = self.client.post(url, data, format="json")
    ...
    def test_signup_duplicate_user_with_valid_data(self):
        ...
        data = {"username": "user1", "password": "Pa$$word123", "email": "kalulearthur@gmail.com"}
        url = reverse("signup")
        response = self.client.post(url, data, format="json")
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.data['errors']['email'], ['This field must be unique.'])
        self.assertEqual(response.data['errors']['username'], ['This field must be unique.'])
